...
describe("Task Lifecycle Integration Tests", () => {
  describe("Weekly task workflow", () => {
    it("should complete weekly task 3 times per week and hide after limit reached", () => {
      ...
      let shouldShow =
        task.todayCount > 0 ||
        task.weekWeekdayCount + task.weekWeekendCount < task.frequency;
  ...
  describe("Weekday-only task workflow", () => {
    it("should only show on weekdays and track completions correctly", () => {
      ...
      testDays.forEach(({ day, isWeekday, dayName }) => {
        const shouldShow =
          isWeekday &&
          (task.todayCount > 0 || task.weekWeekdayCount < task.frequency);
      ...
      const mondayShow =
        true && (task.todayCount > 0 || task.weekWeekdayCount < task.frequency);
  ...
  describe("Finite task workflow", () => {
    it("should track total completions and hide when limit reached", () => {
      ...
      completions.forEach(({ date, increment }, index) => {
        ...
        const shouldShow =
          task.todayCount > 0 || task.totalCount < task.frequency;
  ...
  describe("Weekend-only task workflow", () => {
    it("should only show on weekends", () => {
      ...
      testDays.forEach(({ day, isWeekend, dayName }) => {
        const shouldShow =
          isWeekend &&
          (task.todayCount > 0 || task.weekWeekendCount < task.frequency);
      ...
      const visibility = tasks.map((task) => {
        switch (task.type) {
          ...
          case "monthly":
            return {
              id: task.id,
              show: (task.monthCount ?? 0) < (task.frequency ?? 1),
            };
          case "weekdays":
            return {
              id: task.id,
          ...
          case "finite":
            return {
              id: task.id,
              show: (task.totalCount ?? 0) < (task.frequency ?? 1),
            };
          default:
            return { id: task.id, show: true };
        }
  ...
  describe("Completion and uncompletion cycle", () => {
    it("should handle completing and uncompleting tasks", () => {
      ...
      let shouldShow =
        task.todayCount > 0 || task.weekWeekdayCount + 0 < task.frequency;
      expect(shouldShow).toBe(true);
  ...
  describe("Reward giving workflow", () => {
    it("should give reward when completing task at milestone", () => {
      ...
      expect(
        task.totalCount === 5 ||
          (task.totalCount > 0 && task.totalCount % 5 === 0),
      ).toBe(false);
    ...
    it("should respect maxRewards limit for periodic rewards", () => {
      ...
      const achieved1 =
        task.totalCount > 0 &&
        task.totalCount % 5 === 0 &&
        ((task.maxRewards !== null && task.maxRewards <= task.totalCount / 5) ||
      ...
      expect(
        task.totalCount > 0 &&
          task.totalCount % 5 === 0 &&
          (2 <= task.totalCount / 5 || task.maxRewards === null),
      ).toBe(true);
